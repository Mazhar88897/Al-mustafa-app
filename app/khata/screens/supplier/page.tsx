...
export default function CustomerPage() {
  ...
  const {globalData, setSupp, Supp, user,AccountId, selectedValue, Cust, setGlobalData, setselectedAccountId } = useGlobalStore();
  ...
  const HandleClick = (item: any) =>{
    setSupp(item._id);
    router.push("/khata/screens/supplier/singleSupplier"); 
    // console.log(Cust);
  }
  const [customers, setCustomers] = useState(globalData.suppliers || [] );
...
//   }
  ...
  const saveData = async () => {
    ...
    const postUrl = 'https://khata-backend-express.vercel.app/api/suppliers';  // Replace with your actual API endpoint
...
(customers || []).forEach((record: Record) => {
  ...
  return (
    <div className="p-4">
      {/* <Button onClick={()=>{console.log(AccountId);console.log(globalData);}} className='bg-black'>concole</Button> */}
